...
public class GameUI extends JFrame implements Serializable {
	...
	public GameUI(ContinueLater cl) {
		...
		// System.out.println(myTask.getSeconds());
		...
		btnGuess.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < equation.getLength(); i++) {
					if (guess[i] == null) {
						JOptionPane.showMessageDialog(null, "L�tfen bo�luk b�rakmay�n.");
						return;
					}
				...
				if (numberOfOperators == 0 || numberOfEqualsSign != 1) {
					JOptionPane.showMessageDialog(null, "Tahmininizde en az bir adet matematiksel i�aret ve bir adet e�ittir i�areti bulunmak zorundad�r.");
					return;
				}
				...
				if (Calculate.isEquationValid(temp)) {
					JOptionPane.showMessageDialog(null, "Denklem e�it de�il.");
					return;
				}
				...
				if (currentRow == 5) {
					// Son tahmin hakk�nda da yanl�� girildi.
	...
	public static void addChar(Character newChar) {
		...
		if (cursor == 0 && (newChar == '+' || newChar == '-' || newChar == '*' || newChar == '/' || newChar == '=')) {
			JOptionPane.showMessageDialog(null, "İlk karakter sayı olmalıdır.");
			return;
		}
